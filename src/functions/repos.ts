...
const initializeRepos = (list: RepoList): void => {
    ...
    list.forEach((repo) => {
        ...
        const card = `
            <div class="flex items-center justify-between gap-4 px-4 py-2 border border-stone-800 bg-stone-900">
                <span class="text-base">${name}</span>
                <div class="flex flex-row gap-4 items-center justify-center">
                    <a href="https://github.com/m00nbyte/${name}" target="_blank" title="${name}">
                        <span class="icon-[devicon--github] invert text-xl mt-1.5"></span>
                    </a>
                    ${
                        npm
                            ? `<a href="https://npmjs.com/package/@m00nbyte/${name}" target="_blank" title="${name}">
                                    <span class="icon-[devicon--npm] text-xl mt-1.5"></span>
                                </a>`
                            : ''
                    }
                    ${
                        repo.link
                            ? `<a href="${repo.link.url}" target="_blank" title="${repo.link.title}">
                                    <span class="${
